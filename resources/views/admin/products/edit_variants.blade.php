...
@section('content')
    ...
    <div class="container mt-5">
        <div class="card shadow-sm">
            ...
            <div class="card-body">
                <form action="{{ route('admin.products.update_variants', $product->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" id="variant_sku" name="variant_sku"
                            ...
                            value="{{ old('variant_sku', $product->variant_sku) }}">
                        @error('variant_sku')
                    ...
                    <div class="mb-3">
                        ...
                        <select id="ram" name="ram" class="form-select @error('ram') is-invalid @enderror">
                            ...
                            <option value="16" {{ old('ram', $product->ram) == 16 ? 'selected' : '' }}>16 GB</option>
                            <option value="32" {{ old('ram', $product->ram) == 32 ? 'selected' : '' }}>32 GB</option>
                            <option value="64" {{ old('ram', $product->ram) == 64 ? 'selected' : '' }}>64 GB</option>
                        </select>
                        @error('ram')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-3">
                        ...
                        <select id="storage" name="storage" class="form-select @error('storage') is-invalid @enderror">
                            ...
                            <option value="128" {{ old('storage', $product->storage) == 128 ? 'selected' : '' }}>128 GB</option>
                            <option value="256" {{ old('storage', $product->storage) == 256 ? 'selected' : '' }}>256 GB</option>
                            <option value="512" {{ old('storage', $product->storage) == 512 ? 'selected' : '' }}>512 GB</option>
                        </select>
                        @error('storage')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-5">
                        ...
                        @if ($product->image)
                            <img src="{{ asset('storage/' . $product->image) }}" alt="Hình ảnh biến thể" class="rounded mt-2" width="80" height="80">
                        @endif
                    </div>

                    <div class="d-grid">
                        <button type="submit" class="btn btn-primary">Sửa biến thể</button>
                        <a href="{{ route('admin.products.show', $product->id) }}" class="btn btn-secondary mt-2">Quay lại</a>
                    </div>
                </form>
            </div>
        </div>
